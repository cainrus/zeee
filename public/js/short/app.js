...
(function () {
  ...
  var LastUrlsPanel = Backbone.View.extend({
    ...
    render: function () {
      ...
      _.each(this.collection.models.reverse(), function (model) {
        messageManager.addUrlMsg({
          short: model.get('synonm'),
          count: model.get('count'),
          title: model.get('source'),
          time: parseInt(model.get('created'))
        });
      }, this);
  ...
  var messageManagerClass = function () {
    ...
    var add = function (type, data) {
      ...
      removeExcess(type, function () {
        ...
        newMsg.animate({top: 0, opacity: 1}, {
    ...
    var animateMascott1 = function () {
      ...
      mascott.animate({top: '110px', opacity: 0.9, right: right - 20}, {
        easing: 'easeInOutExpo',
        queue: false,
        duration: 800,
        complete: function () {
  ...
  $(function () {
    ...
    if (!$.browser.mobile && window.chrome && !chrome.app.isInstalled) {
      ...
      dispatcher.trigger('eventPanel.add', 'Simply click the icon to install extension: <span id="installChromeExt" style="cursor:pointer;position: absolute;" class="chrome-ext-icon"></span>', 'success', {
        data: {classes: 'pinned alert alert-success chrome-extension'},
        status: 'success',
        title: 'Google Chrome Extension availible.'
      });
